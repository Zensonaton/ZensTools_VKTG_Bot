...
async def document_handler(msg: types.Message):
	...
	privateKey = open("BLKeys/Private key", "r").read()
	secretPhrase = open("BLKeys/Secret phrase", "r").read()

	emsg   = pgpy.PGPMessage.from_blob(fileContents)
	key, _ = pgpy.PGPKey.from_blob(privateKey)

	with key.unlock(secretPhrase):
